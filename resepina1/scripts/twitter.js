twitter_feed = {
    ...
    timeAgo: function(dateString) {
		...
		if (diff < second * 2) {					return "This tweet was just posted";		}
		if (diff < minute) {						return "Posted about " + Math.floor(diff / second) + " seconds ago";}
		if (diff < minute * 2) {					return "Posted about 1 minute ago";	}
		if (diff < hour) {							return "Posted about " + Math.floor(diff / minute) + " minutes ago";	}
		if (diff < hour * 2) {						return "Posted about 1 hour ago";		}
		if (diff < day) {							return  Math.floor(diff / hour) + " hours ago";		}
		if (diff > day && diff < day * 2) {			return "Posted sometime yesterday";		}
		if (diff < day * 365) {						return "Posted about " + Math.floor(diff / day) + " days ago";	}
		else {
